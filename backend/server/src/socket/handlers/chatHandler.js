...
module.exports = (io, socket) => {
    ...
    socket.on('chat:edit', async (data) => {
        try {
            ...
            io.to(`conversation:${updatedMessage.conversationId}`).emit('chat:message:updated', updatedMessage);
        } catch (error) {
    ...
    socket.on('chat:delete', async (messageId) => {
        try {
            ...
            io.to(`conversation:${deletedMessage.conversationId}`).emit('chat:message:deleted', { messageId });
        } catch (error) {
    ...
    socket.on('chat:read', async ({ conversationId, messageId }) => {
        try {
            ...
            io.to(`conversation:${conversationId}`).emit('chat:read', { messageId, userId: socket.userId });
