...
exports.getProductAnalytics = asyncHandler(async (req, res) => {
    ...
    const slowMoving = productData.filter(p =>
        new Date(p.createdAt) < thirtyDaysAgo && p.sales < 5
    ).sort((a, b) => a.sales - b.sales).slice(0, 10);

    // 4. Top Performing (by Profit)
    const topByProfit = [...productData]
    ...
    const avgProfitMargin = productData.length > 0
        ? (productData.reduce((sum, p) => sum + (p.revenue > 0 ? (p.profit / p.revenue) : 0), 0) / productData.length) * 100
        : 0;
...
exports.exportSalesReport = asyncHandler(async (req, res) => {
    ...
    const csvRows = [
        ['Order Number', 'Date', 'Customer', 'Email', 'Payment Method', 'Status', 'Subtotal', 'Discount', 'Total'].join(',')
    ];

    orders.forEach(order => {
        const customerName = order.user ? `${order.user.firstName} ${order.user.lastName}` : (order.guestInfo?.name || order.walkInName || 'Guest');
...
exports.getTaxAnalytics = asyncHandler(async (req, res) => {
    ...
    orders.forEach(order => {
        ...
        try {
            const address = typeof order.billingAddress === 'string'
...
exports.getSiteAnalytics = asyncHandler(async (req, res) => {
    ...
    const realTimeVisitorsCount = await prisma.analyticsEvent.groupBy({
        by: ['sessionId'],
        where: {
    ...
    return successResponse(res, {
        data: {
            ...
            geoDistribution: Object.entries(geoDistributionMap)
                .map(([name, value]) => ({ name, value }))
                .sort((a, b) => b.value - a.value),
            deviceDistribution: Object.entries(deviceDistributionMap)
                .map(([name, value]) => ({ name, value }))
        },
