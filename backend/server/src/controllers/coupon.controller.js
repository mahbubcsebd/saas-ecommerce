...
exports.createCoupon = async (req, res) => {
    try {
        const {
            code, name, description, type, value, maxDiscountCap,
            applicableOn, categoryIds, productIds, excludedProducts,
            minOrderValue, usageLimit, perUserLimit,
            newUsersOnly, allowedUserIds, allowedCountries,
        ...
        if (error.code === 'P2002') return res.status(409).json({ success: false, message: 'A coupon with this code already exists.' });
...
exports.updateCoupon = async (req, res) => {
    try {
        ...
        const {
            name, description, type, value, maxDiscountCap,
            applicableOn, categoryIds, productIds, excludedProducts,
            minOrderValue, usageLimit, perUserLimit,
            newUsersOnly, allowedUserIds, allowedCountries,
            startDate, endDate, isActive,
        ...
        const updated = await prisma.discount.update({
            ...
            data: {
                ...
                ...(maxDiscountCap !== undefined && { maxDiscountCap: maxDiscountCap ? parseFloat(maxDiscountCap) : null }),
...
exports.validateCoupon = async (req, res, next) => {
    try {
        ...
        res.json({ success: true, discount: result.discountAmount, coupon: { code: result.coupon.code, name: result.coupon.name, type: result.coupon.type, value: result.coupon.value } });
