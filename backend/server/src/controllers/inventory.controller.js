...
exports.adjustStock = async (req, res, next) => {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const qtyChange = type === 'ADD' ? quantity : (type === 'REMOVE' ? -quantity : newQty - previousQty);
        ...
        try {
            const productName = variantId ? `a variant of ${result?.name || 'Product'}` : (result?.name || 'Product');
