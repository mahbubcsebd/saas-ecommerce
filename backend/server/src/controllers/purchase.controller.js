            ...
            if (newPurchase.status === 'RECEIVED') {
                for (const item of items) {
                    ...
                    if (item.variantId) {
                        ...
                        const parent = await tx.product.findUnique({ where: { id: item.productId }, select: { stock: true } });
        ...
        try {
            if (backInStockProducts.size > 0) {
                ...
                for (const pid of backInStockProducts) {
                    const product = await prisma.product.findUnique({ where: { id: pid }, select: { id: true, name: true, slug: true } });
                    if (!product) continue;
                    const wishlists = await prisma.wishlist.findMany({ where: { productId: pid }, select: { userId: true } });
