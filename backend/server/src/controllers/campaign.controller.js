...
exports.createCampaign = async (req, res) => {
    try {
        const { name, subject, type, content, targetType, targetGroupIds, targetUserIds, scheduledAt } = req.body;
...
exports.updateCampaign = async (req, res) => {
    try {
        const { id } = req.params;
        const { name, subject, type, content, targetType, targetGroupIds, targetUserIds, scheduledAt, status } = req.body;

        const existing = await prisma.campaign.findUnique({ where: { id } });
        if (!existing) return res.status(404).json({ success: false, message: 'Campaign not found' });

        if (existing.status === 'SENT' || existing.status === 'SENDING') {
            return res.status(400).json({ success: false, message: 'Cannot update a campaign that is already sending or sent' });
        }
...
exports.deleteCampaign = async (req, res) => {
    try {
        ...
        if (existing.status === 'SENDING') {
            return res.status(400).json({ success: false, message: 'Cannot delete a campaign while it is sending' });
        }
...
exports.sendCampaign = async (req, res) => {
    try {
        ...
        if (campaign.status === 'SENT' || campaign.status === 'SENDING') {
            return res.status(400).json({ success: false, message: 'Campaign already processing or finished' });
        }
...
exports.sendQuickEmail = async (req, res) => {
  try {
    ...
    if (!subject || !templateHtml) {
      return res.status(400).json({ success: false, message: 'subject and templateHtml are required' });
    }
    ...
    const resolvedAttachments = await Promise.all(
      attachments.map(async (att) => {
        try {
          ...
          return { filename: att.filename, content: buffer, contentType: att.contentType || 'application/octet-stream' };
        } catch (e) {
...
exports.getRecipients = async (req, res) => {
  try {
    ...
    const users = await prisma.user.findMany({
      where: filter,
      select: { id: true, email: true, firstName: true, lastName: true, role: true, status: true, groupId: true },
