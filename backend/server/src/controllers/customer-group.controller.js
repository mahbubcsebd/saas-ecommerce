...
const createCustomerGroup = async (req, res) => {
  try {
    ...
    if (error.code === 'P2002') {
      return res
        .status(409)
        .json({ success: false, message: 'A group with this name already exists.' });
    }
    console.error('createCustomerGroup error:', error);
    return res
      .status(500)
      .json({ success: false, message: 'Server error creating customer group.' });
  }
};
...
const getAllCustomerGroups = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: 'Server error fetching customer groups.' });
  }
};
...
const getCustomerGroup = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: 'Server error fetching customer group.' });
  }
};
...
const updateCustomerGroup = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: 'Server error updating customer group.' });
  }
};
...
const deleteCustomerGroup = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: 'Server error deleting customer group.' });
  }
};
...
const removeUserFromGroup = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: 'Server error removing user from group.' });
  }
};
