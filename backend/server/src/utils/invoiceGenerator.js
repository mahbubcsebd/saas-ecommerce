...
const generateInvoicePDF = (doc, data) => {
    ...
    // Right: Invoice Meta
    // "INVOICE" Title
    doc.fontSize(24).font(fonts.bold).fillColor(colors.primary) // Very light gray like frontend
    ...
    // Invoice #
    doc.font(fonts.bold).text(`#${invoice.invoiceNumber}`, 0, metaY, { align: 'right', width: doc.page.width - layout.margin });
    metaY += 16; /* Increased line height */

    // Date
    doc.font(fonts.regular)
       .text(`Date: ${new Date(invoice.issueDate).toLocaleDateString()}`, 0, metaY, { align: 'right', width: doc.page.width - layout.margin });
    metaY += 16;

    // Status
    doc.text(`Status: ${order.paymentStatus}`, 0, metaY, { align: 'right', width: doc.page.width - layout.margin });
    ...
    if (customerEmail) { doc.text(customerEmail, layout.margin, y); y += 14; }
    if (customerPhone) { doc.text(customerPhone, layout.margin, y); }
    ...
    order.items.forEach((item, i) => {
        ...
        if (y + rowHeight > doc.page.height - 150) { // Reserve 150 for Footer area to be safe
            doc.addPage();
            y = 50;
        ...
        // Name
        doc.fontSize(10).font(fonts.bold).text(productName, layout.margin + 10, y + 8, { width: 250 });
        let textY = y + 20;

        if (variantName) {
            doc.fontSize(9).font(fonts.regular).text(variantName, layout.margin + 10, textY);
            textY += 12;
        }
        if (sku) {
            doc.fontSize(8).font(fonts.regular).text(sku, layout.margin + 10, textY);
        }

        doc.fontSize(10).font(fonts.regular).text(item.quantity.toString(), colQty, y + 12, { width: 50, align: 'center' });
        ...
        doc.font(fonts.bold).text(formatCurrency(total), colTotal, y + 12, { width: 70, align: 'right' });

        y += rowHeight;

        // Line
        doc.moveTo(layout.margin, y).lineTo(doc.page.width - layout.margin, y).strokeColor('#e5e7eb').lineWidth(0.5).stroke();
    });
    ...
    doc.fontSize(8).font(fonts.regular)
       .text('Payment is due within 15 days. Please check the goods upon delivery. Returns accepted within 7 days with original receipt.', layout.margin, footerY + 15, { width: 250 });
    ...
    doc.fontSize(9).font(fonts.regular).fillColor(colors.text)
       .text('Thank you for your business!', layout.margin, bottomTextY, { align: 'center', width: doc.page.width - (layout.margin * 2) });
};
