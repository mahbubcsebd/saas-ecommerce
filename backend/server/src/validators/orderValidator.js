...
const createOrderValidation = [
  ...
  // POS Specific Fields
  body('walkInName').optional().isString().trim(),

  body('walkInPhone').optional().isString().trim(),
  ...
  body('vatPercent').optional().isFloat({ min: 0 }).withMessage('VAT must be a positive number'),
...
const orderIdValidation = [param('id').isMongoId().withMessage('Invalid Order ID format')];
