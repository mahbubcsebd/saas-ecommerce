...
async function runTest() {
  ...
  try {
      ...
      let createdUserId;
      try {
          const res = await axios.post(`${API_URL}/admin/users`, newUserPayload, { headers: authHeader });
      ...
      try {
          ...
          if (res.data.data.role !== 'STAFF') console.log('⚠️ WARNING: Role mismatch', res.data.data.role);
      } catch (e) {
      ...
      try {
          const res = await axios.patch(`${API_URL}/user/${createdUserId}`, updatePayload, { headers: authHeader });
