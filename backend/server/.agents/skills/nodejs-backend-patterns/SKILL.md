...
When maintaining this Node.js backend, always follow this workflow if the application (Nodemon) crashes or if `500 Internal Server Error` occurs:

1. **DO NOT GUESS**: Never run generic terminal commands or guess the issue immediately.
2. **CHECK THE LOGS**: The system is equipped with Winston. All fatal crashes (`uncaughtException`, `unhandledRejection`) and runtime errors will be logged with full stack traces.
3. **TOOL USAGE**: Immediately use `view_file` or `grep_search` on `backend/server/logs/error.log` to read the tail of the error log.
4. **IDENTIFY**: Find the exact file and line number from the stack trace inside the log.
