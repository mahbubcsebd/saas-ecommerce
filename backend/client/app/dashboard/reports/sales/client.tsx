...
export default function SalesReportClient() {
    ...
    const fetchAnalytics = async () => {
        ...
        try {
            ...
            const response = await fetch(`${BACKEND_URL}/analytics/advanced?startDate=${startStr}&endDate=${endStr}&groupBy=${groupBy}`, {
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const response = await fetch(`${BACKEND_URL}/analytics/export?startDate=${startStr}&endDate=${endStr}`, {
                headers: { 'Authorization': `Bearer ${session.accessToken}` }
            });
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        Detailed breakdown of your store performance from {format(dateRange.start, 'PP')} to {format(dateRange.end, 'PP')}
                    </p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <Button onClick={handleExport} variant="outline" className="gap-2 border-primary/20 hover:bg-primary/5">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card className="border-none shadow-md bg-gradient-to-br from-blue-500/10 to-blue-600/5">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{formatCurrency(data?.kpi?.totalRevenue || 0)}</div>
                ...
                <Card className="border-none shadow-md bg-gradient-to-br from-emerald-500/10 to-emerald-600/5">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{data?.kpi?.totalOrders || 0}</div>
                ...
                <Card className="border-none shadow-md bg-gradient-to-br from-amber-500/10 to-amber-600/5">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{formatCurrency(data?.kpi?.averageOrderValue || 0)}</div>
                ...
                <Card className="border-none shadow-md bg-gradient-to-br from-purple-500/10 to-purple-600/5">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{data?.kpi?.totalItemsSold || 0}</div>
                        <p className="text-xs text-muted-foreground mt-1">Products shipped</p>
                    </CardContent>
                </Card>
            </div>
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                ...
                <Card className="lg:col-span-2 shadow-md">
                    <CardHeader>
                        ...
                        <CardDescription>Visualizing revenue growth over the selected interval</CardDescription>
                    </CardHeader>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={data?.timeSeriesData || []}>
                                    ...
                                    <XAxis
                                        dataKey="time"
                                        tickFormatter={(val) => {
                                            if (groupBy === 'day') return format(new Date(val), 'MMM d');
                                            if (groupBy === 'hour') return format(new Date(val), 'HH:mm');
                                            return val;
                                        }}
                        ...
                        <div className="mt-4 space-y-2">
                            {data?.categories?.slice(0, 3).map((cat: any, i: number) => (
                                <div key={cat.name} className="flex items-center justify-between text-sm">
                                    ...
                                    <span className="text-muted-foreground">{formatCurrency(cat.revenue)}</span>
                                </div>
                            ))}
                        </div>
                    </CardContent>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                <Card className="lg:col-span-2 shadow-md overflow-hidden">
                    <CardHeader className="bg-muted/30">
                        ...
                        <CardDescription>Best sellers ranked by total revenue generated</CardDescription>
                    </CardHeader>
                                ...
                                <tbody className="divide-y">
                                    {data?.topProducts?.map((product: any, idx: number) => (
                                        <tr key={product.id} className="hover:bg-muted/20 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right">{product.sales} units</td>
                                            <td className="px-6 py-4 text-right font-bold text-primary">{formatCurrency(product.revenue)}</td>
                                        </tr>
                                    ))}
                ...
                <Card className="shadow-md">
                    <CardHeader>
                        ...
                        <CardDescription>Volume handled by each payment channel</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="h-[300px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                        ...
                        <div className="mt-4 pt-4 border-t space-y-3">
                            {Object.entries(data?.statusStats || {}).map(([status, count]: any) => (
                                <div key={status} className="flex items-center justify-between text-sm">
                                    <span className="text-muted-foreground capitalize">{status.toLowerCase()} Orders</span>
