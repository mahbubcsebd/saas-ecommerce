...
export default function TaxReportClient() {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      'download',
      `tax_report_${format(dateRange.start, 'yyyy-MM-dd')}_to_${format(dateRange.end, 'yyyy-MM-dd')}.csv`,
    );
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            VAT and tax collection from {format(dateRange.start, 'PP')} to{' '}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="border-none shadow-md bg-gradient-to-br from-blue-600/10 to-blue-700/5">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(data?.kpi?.totalTaxCollected || 0)}
            </div>
        ...
        <Card className="border-none shadow-md bg-gradient-to-br from-emerald-600/10 to-emerald-700/5">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(data?.kpi?.totalTaxableRevenue || 0)}
            </div>
      ...
      <Card className="shadow-md">
        <CardHeader>
          ...
          <CardDescription>
            Recent orders with tax breakdowns for accounting
          </CardDescription>
        </CardHeader>
                ...
                {(data?.detailedReport || []).map((order: any) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      {formatCurrency(order.taxableAmount)}
                    </td>
                    <td className="px-6 py-4 text-right">
                      {order.taxPercent}%
                    </td>
