...
const AVAILABLE_PERMISSIONS = [
    ...
    { id: 'manage_categories', label: 'Manage Categories', category: 'Inventory' },
...
export default function StaffClient() {
    ...
    const handleUpdatePermissions = async () => {
        ...
        try {
            const BACKEND_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api';
            const res = await fetch(`${BACKEND_URL}/staff/${selectedStaff.id}/permissions`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${session?.accessToken}`,
                    'Content-Type': 'application/json'
    ...
    const filteredStaff = staff.filter(s => {
        const matchesSearch = `${s.firstName} ${s.lastName} ${s.email}`.toLowerCase().includes(searchQuery.toLowerCase());
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="space-y-4">
                        ...
                        <h1 className="text-5xl md:text-6xl font-[1000] tracking-tighter leading-none italic uppercase">
                            Team <span className="text-blue-500 text-outline-white">Command</span>
                        </h1>
                        <p className="text-slate-400 font-medium max-w-xl text-lg leading-relaxed">
                            Master your workforce with granular role hierarchies, specialized permission matrices, and real-time operational auditing.
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        ...
                        >
                            <UserPlus className="w-5 h-5 group-hover:scale-110 transition-transform" /> Recruit Staff
                        </Button>
                        <Button
                            variant="outline"
            ...
            <Tabs defaultValue="members" className="w-full">
                <TabsList className="bg-slate-100 p-1.5 rounded-3xl h-16 w-full md:w-auto flex justify-start mb-8 gap-1 shadow-sm">
                    <TabsTrigger value="members" className="rounded-2xl px-10 h-13 font-black text-xs uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-blue-600 data-[state=active]:shadow-lg transition-all">
                        <UserCheck className="w-4 h-4 mr-2" /> Members
                    </TabsTrigger>
                    <TabsTrigger value="activity" className="rounded-2xl px-10 h-13 font-black text-xs uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-blue-600 data-[state=active]:shadow-lg transition-all">
                    ...
                    <div className="flex flex-col md:flex-row gap-4 items-center mb-8">
                        ...
                        <Select value={roleFilter} onValueChange={setRoleFilter}>
                            ...
                            <SelectContent className="rounded-2xl border-none shadow-2xl p-2">
                                <SelectItem value="ALL" className="rounded-xl font-bold py-3 text-xs uppercase">All Roles</SelectItem>
                                <SelectItem value="SUPER_ADMIN" className="rounded-xl font-bold py-3 text-xs uppercase">Super Admin</SelectItem>
                                <SelectItem value="ADMIN" className="rounded-xl font-bold py-3 text-xs uppercase">Admin</SelectItem>
                                <SelectItem value="MANAGER" className="rounded-xl font-bold py-3 text-xs uppercase">Manager</SelectItem>
                                <SelectItem value="STAFF" className="rounded-xl font-bold py-3 text-xs uppercase">Staff</SelectItem>
                            </SelectContent>
                        </Select>
                    </div>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {filteredStaff.map(member => (
                                        <tr key={member.id} className="group hover:bg-slate-50/50 transition-all duration-300">
                                            ...
                                            <td className="px-8 py-6">
                                                <Badge className={`rounded-lg font-[900] text-[10px] uppercase tracking-widest px-3 py-1 border-none ${
                                                    member.role === 'SUPER_ADMIN' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' :
                                                    member.role === 'ADMIN' ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' :
                                                    member.role === 'MANAGER' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-100' :
                                                ...
                                                {member.customRole && (
                                                    <div className="mt-1 flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200 w-fit">
                                                        ...
                                                        <span className="text-[9px] font-black uppercase text-slate-600">{member.customRole.name}</span>
                                                    </div>
                                                )}
                                            </td>
                                            <td className="px-8 py-6">
                                                <div className="flex items-center gap-2">
                                                    <div className={`w-2 h-2 rounded-full ${
                                                        ...
                                                        member.status === 'PENDING' ? 'bg-amber-500 animate-pulse' : 'bg-slate-300'
                                                    }`} />
                                                ...
                                                <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                                    ...
                                                    <DropdownMenu>
                                                        <DropdownMenuTrigger asChild>
                                                            <Button variant="ghost" size="icon" className="h-12 w-12 rounded-xl hover:bg-slate-100 border">
                                                        ...
                                                        <DropdownMenuContent align="end" className="rounded-2xl border-none shadow-2xl p-2 min-w-[180px]">
                                                            <DropdownMenuItem className="rounded-xl font-bold py-3 text-xs uppercase tracking-tight">
                                                                <Edit2 className="w-4 h-4 mr-3 text-blue-500" /> Edit Profile
                                                            </DropdownMenuItem>
                                                            <DropdownMenuItem className="rounded-xl font-bold py-3 text-xs uppercase tracking-tight">
                                                                <RefreshCw className="w-4 h-4 mr-3 text-amber-500" /> Reset Password
                                                            </DropdownMenuItem>
                    ...
                    <Card className="rounded-[2.5rem] border-none shadow-[0_32px_64px_-16px_rgba(0,0,0,0.05)] bg-white overflow-hidden">
                        <CardHeader className="p-10 border-b border-slate-50 bg-slate-50/30">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <CardTitle className="text-2xl font-black italic uppercase">Operational Narrative</CardTitle>
                                    <CardDescription className="text-sm font-medium text-slate-400">Comprehensive ledger of staff interactions with systemic assets.</CardDescription>
                                </div>
                                <Button variant="outline" onClick={() => fetchData()} className="ml-auto rounded-xl border-2 font-black text-[10px] uppercase tracking-widest h-12 hover:bg-blue-50 hover:text-blue-600 transition-all">
                            ...
                            <div className="divide-y divide-slate-50">
                                {activities.length === 0 ? (
                                    <div className="py-24 text-center">
                                        ...
                                        <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">No activity logs recorded today.</p>
                                    </div>
                                ) : (
                                    activities.map(log => (
                                        <div key={log.id} className="p-8 hover:bg-slate-50/50 transition-all flex items-start gap-6">
                                            ...
                                            <div className="flex-1 space-y-2">
                                                <div className="flex items-center justify-between">
                                                    <p className="text-sm font-[900] text-slate-900">
                                                        ...
                                                        <Badge variant="outline" className="rounded-lg font-black text-[9px] uppercase tracking-tighter bg-emerald-50 text-emerald-600 border-emerald-100">
                                                            {log.action}
                                                        </Badge>
                                                    </p>
                                                ...
                                                <p className="text-xs font-bold text-slate-500 italic">
                                                    Modified resource: <span className="text-slate-900 not-italic font-black bg-slate-100 px-2 py-0.5 rounded leading-loose">{log.target}</span>
                                                </p>
                                                <div className="flex items-center gap-4 pt-1">
                                                    <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest flex items-center gap-1">
                                                        <Globe className="w-3 h-3" /> {log.ipAddress || 'Internal Trace'}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
            ...
            <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
                <DialogContent className="sm:max-w-xl rounded-[2.5rem] p-10 border-none shadow-[0_32px_64px_-16px_rgba(0,0,0,0.2)]">
                    <DialogHeader className="mb-8">
                        <DialogTitle className="text-3xl font-[1000] tracking-tight italic uppercase italic">Recruit New Talent</DialogTitle>
                        <DialogDescription className="text-lg font-medium text-slate-400 leading-relaxed">Send an encrypted setup link to authorize a new staff member.</DialogDescription>
                    </DialogHeader>

                    <form onSubmit={handleInvite} className="space-y-6">
                        <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <Label className="text-[10px] font-black uppercase text-slate-500 pl-1">First Name</Label>
                                <Input
                            ...
                            <div className="space-y-2">
                                <Label className="text-[10px] font-black uppercase text-slate-500 pl-1">Last Name</Label>
                                <Input
                                    className="h-14 rounded-2xl bg-slate-50 border-none focus-visible:ring-2 focus-visible:ring-blue-500/20 font-bold"
                        ...
                        <div className="space-y-2">
                            <Label className="text-[10px] font-black uppercase text-slate-500 pl-1">Official Workspace Email</Label>
                            <Input
                                type="email"
                                className="h-14 rounded-2xl bg-slate-50 border-none focus-visible:ring-2 focus-visible:ring-blue-500/20 font-bold"
                        ...
                        <div className="space-y-2">
                            <Label className="text-[10px] font-black uppercase text-slate-500 pl-1">Operational Role assignment</Label>
                            <Select
                            ...
                            >
                                ...
                                <SelectContent className="rounded-2xl border-none shadow-2xl p-2">
                                    <SelectItem value="STAFF" className="rounded-xl font-bold py-3 text-xs uppercase">Field Staff</SelectItem>
                                    <SelectItem value="MANAGER" className="rounded-xl font-bold py-3 text-xs uppercase">Department Manager</SelectItem>
                                    <SelectItem value="ADMIN" className="rounded-xl font-bold py-3 text-xs uppercase">System Administrator</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                        ...
                        {customRoles.length > 0 && (
                            <div className="space-y-2">
                                <Label className="text-[10px] font-black uppercase text-slate-500 pl-1">Matrix Role (Custom)</Label>
                                <Select
                                    value={inviteForm.customRoleId}
                                    onValueChange={val => setInviteForm({...inviteForm, customRoleId: val})}
                                >
                                    <SelectTrigger className="h-14 rounded-2xl bg-slate-50 border-none px-6 font-black text-xs uppercase tracking-widest">
                                        <SelectValue placeholder="Selective Matrix Alignment" />
                                    </SelectTrigger>
                                    <SelectContent className="rounded-2xl border-none shadow-2xl p-2">
                                        <SelectItem value="NONE" className="rounded-xl font-bold py-3 text-xs uppercase">No Custom Role</SelectItem>
                                        {customRoles.map(role => (
                                            <SelectItem key={role.id} value={role.id} className="rounded-xl font-bold py-3 text-xs uppercase italic">{role.name}</SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                            </div>
                        ...
                        <DialogFooter className="mt-10 pt-6">
                            <Button type="button" variant="ghost" onClick={() => setIsAddModalOpen(false)} className="rounded-2xl font-black text-xs uppercase tracking-widest px-8">Discard</Button>
                            <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white rounded-2xl px-10 h-16 font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-blue-500/20 transition-all">
            ...
            <Dialog open={isPermModalOpen} onOpenChange={setIsPermModalOpen}>
                <DialogContent className="sm:max-w-2xl rounded-[2.5rem] p-10 border-none shadow-[0_32px_64px_-16px_rgba(0,0,0,0.2)]">
                    <DialogHeader className="mb-8">
                        <DialogTitle className="text-3xl font-[1000] tracking-tight italic uppercase">Access Matrix Control</DialogTitle>
                        <DialogDescription className="text-lg font-medium text-slate-400">Define granular modular access for <span className="text-slate-900 font-black">{selectedStaff?.firstName} {selectedStaff?.lastName}</span>.</DialogDescription>
                    </DialogHeader>
                                    ...
                                    {AVAILABLE_PERMISSIONS.filter(p => p.category === cat).map(perm => (
                                        ...
                                        >
                                            <div className="space-y-0.5">
                                                <p className="text-xs font-black uppercase tracking-tight">{perm.label}</p>
                                                <p className={`text-[9px] font-bold uppercase transition-colors ${selectedPermissions.includes(perm.id) ? 'text-blue-100' : 'text-slate-300'}`}>
                                                    System Node: {perm.id}
                                                </p>
                                            </div>
                                            <div className={`h-6 w-6 rounded-lg flex items-center justify-center transition-all ${
                                                selectedPermissions.includes(perm.id) ? 'bg-white text-blue-600' : 'bg-slate-50 text-slate-200 group-hover:bg-blue-50'
                                            }`}>
                                                {selectedPermissions.includes(perm.id) ? <CheckCircle2 className="w-4 h-4" /> : <Shield className="w-4 h-4 text-slate-200" />}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                    ...
                    <DialogFooter className="mt-10 pt-6">
                        <Button type="button" variant="ghost" onClick={() => setIsPermModalOpen(false)} className="rounded-2xl font-black text-xs uppercase tracking-widest px-8">Discard</Button>
                        <Button onClick={handleUpdatePermissions} className="bg-slate-900 hover:bg-slate-800 text-white rounded-2xl px-12 h-16 font-black text-xs uppercase tracking-widest shadow-xl transition-all">
                            Synchronize Matrix
                        </Button>
                    </DialogFooter>
                </DialogContent>
