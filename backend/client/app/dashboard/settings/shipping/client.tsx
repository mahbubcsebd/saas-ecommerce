...
export default function ShippingSettingsClient() {
    ...
    const [zoneForm, setZoneForm] = useState<any>({
        name: '', countries: [], regions: [], isActive: true, priority: 0
    });
    const [packagingForm, setPackagingForm] = useState<any>({
        name: '', length: 0, width: 0, height: 0, weight: 0, maxWeight: 0, cost: 0, isActive: true
    });
    ...
    const handlePackagingDelete = async (id: string) => {
        if (!await confirm({
            ...
            message: 'Are you sure you want to delete this packaging type? This will immediately affect shipping cost calculations for orders using this box.',
            type: 'danger',
    ...
    if (isLoading) {
        return (
            <div className="h-[60vh] flex flex-col items-center justify-center space-y-4">
                ...
                <p className="text-sm text-muted-foreground animate-pulse">Loading shipping engine...</p>
            </div>
        );
    }
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-8">
                <div className="space-y-1">
                    ...
                    <h1 className="text-3xl font-bold tracking-tight text-slate-900 uppercase">Shipping Settings</h1>
                    <p className="text-slate-500 text-sm">Configure delivery methods, zones, and logistics parameters.</p>
                </div>

                <div className="flex items-center gap-3 px-4 py-2 bg-emerald-50 border border-emerald-100 rounded-lg">
                    <Truck className="w-4 h-4 text-emerald-600" />
                    <span className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider">Logistics: Active</span>
                </div>
            </div>
            ...
            <Tabs defaultValue="general" className="w-full">
                        ...
                        <CardHeader className="bg-slate-50/50 border-b">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <CardDescription className="text-xs">Main shipping rules for your entire store.</CardDescription>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent className="p-6 space-y-6">
                            <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 border border-slate-100">
                                <div className="space-y-0.5">
                                    <Label className="text-sm font-bold text-slate-700">Enable Shipping</Label>
                                    <p className="text-xs text-slate-500">Enable or disable delivery options on checkout.</p>
                                </div>
                                <Switch
                                    checked={shippingConfig.isShippingEnabled}
                                    onCheckedChange={val => handleUpdateConfig({ isShippingEnabled: val })}
                                />
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Free Shipping Threshold</Label>
                                    ...
                                    <p className="text-[10px] text-muted-foreground">Orders above this value get free shipping (if enabled in rates).</p>
                                </div>

                                <div className="space-y-2">
                                    <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">Calculation Method</Label>
                                    <Select
                                        value={shippingConfig.calculationMethod}
                                        onValueChange={(val: string) => handleUpdateConfig({ calculationMethod: val })}
                                    >
                                        <SelectTrigger>
                                            <SelectValue placeholder="Select method" />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem value="ZONE_BASED">Zone Based (Static)</SelectItem>
                                            <SelectItem value="FLAT_RATE">Fixed Rate (Global)</SelectItem>
                ...
                <TabsContent value="zones" className="space-y-6">
                    <div className="flex items-center justify-between px-1">
                        <div>
                            <h3 className="text-lg font-bold text-slate-900">Shipping Zones</h3>
                            <p className="text-xs text-slate-500">Geographic areas and their specific delivery rates.</p>
                        </div>
                        <Button size="sm" className="h-8 gap-1.5" onClick={() => toast.info('Zone management inherited from core shipping module')}>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {zones.length === 0 ? (
                            <Card className="border-dashed p-12 flex flex-col items-center justify-center text-center space-y-3">
                                ...
                                <p className="font-bold text-slate-700">No Shipping Zones Configured</p>
                            </Card>
                        ) : (
                            zones.map(zone => (
                                <Card key={zone.id} className="shadow-sm hover:border-primary/20 transition-all">
                                    ...
                                    <CardContent className="p-4">
                                        <div className="space-y-2">
                                            {zone.rates?.map((rate: any) => (
                                                <div key={rate.id} className="flex items-center justify-between p-2 rounded-lg bg-slate-50 border border-slate-100 border-dashed">
                                                    <span className="text-xs font-medium text-slate-700">{rate.method} ({rate.carrier})</span>
                                                    <span className="text-xs font-bold text-primary">
                                                        {rate.calculationType === 'FLAT' ? `$${rate.flatRate}` : 'Variable'}
                                                    </span>
                                                </div>
                                            ))}
                                        </div>
                ...
                <TabsContent value="packaging" className="space-y-6">
                    <div className="flex items-center justify-between px-1">
                        <div>
                            <h3 className="text-lg font-bold text-slate-900">Packaging Inventory</h3>
                            <p className="text-xs text-slate-500">Define standard boxes for shipping calculations.</p>
                        </div>
                        <Dialog open={isPackagingDialogOpen} onOpenChange={setIsPackagingDialogOpen}>
                            <DialogTrigger asChild>
                                <Button size="sm" className="h-8 gap-1.5" onClick={() => {
                                    ...
                                    setPackagingForm({ name: '', length: 0, width: 0, height: 0, weight: 0, maxWeight: 0, cost: 0, isActive: true });
                                }}>
                            ...
                            <DialogContent className="sm:max-w-[425px]">
                                <DialogHeader>
                                    <DialogTitle>{editingPackaging ? 'Edit' : 'Add'} Packaging</DialogTitle>
                                </DialogHeader>
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        <div className="space-y-1.5">
                                            ...
                                            <Input
                                                ...
                                                onChange={e => setPackagingForm({ ...packagingForm, length: parseFloat(e.target.value) })}
                                            />
                                        </div>
                                        <div className="space-y-1.5">
                                            <Label className="text-[10px] font-bold uppercase text-slate-500">Width</Label>
                                            <Input
                                                type="number"
                                                value={packagingForm.width}
                                                onChange={e => setPackagingForm({ ...packagingForm, width: parseFloat(e.target.value) })}
                                            />
                                        </div>
                                        <div className="space-y-1.5">
                                            <Label className="text-[10px] font-bold uppercase text-slate-500">Height</Label>
                                            <Input
                                                type="number"
                                                value={packagingForm.height}
                                                onChange={e => setPackagingForm({ ...packagingForm, height: parseFloat(e.target.value) })}
                                            />
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1.5">
                                            <Label className="text-xs font-bold uppercase text-slate-500">Base Cost ($)</Label>
                                            <Input
                                                type="number"
                                                value={packagingForm.cost}
                                                onChange={e => setPackagingForm({ ...packagingForm, cost: parseFloat(e.target.value) })}
                                            />
                                        </div>
                                        <div className="space-y-1.5 flex flex-col justify-end">
                                            <div className="flex items-center gap-2 h-10 px-3 border rounded-md">
                                                ...
                                                <Switch
                                                    ...
                                                    onCheckedChange={val => setPackagingForm({ ...packagingForm, isActive: val })}
                                                />
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {packagings.length === 0 ? (
                            <Card className="col-span-full border-dashed p-12 flex flex-col items-center justify-center text-center space-y-3">
                                ...
                                <p className="font-bold text-slate-700">No Packaging Types Found</p>
                            </Card>
                        ) : (
                            packagings.map(pkg => (
                                <Card key={pkg.id} className="shadow-sm border-slate-200">
                                    ...
                                    <CardContent className="p-4 space-y-3">
                                        <div className="flex items-center justify-between text-xs">
                                            ...
                                            <span className="font-bold text-slate-700">{pkg.length}x{pkg.width}x{pkg.height} cm</span>
                                        </div>
                                        <div className="flex items-center justify-between text-xs">
