...
export default function StoreSettingsClient() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [conRes, legRes] = await Promise.all([
        ...
        fetch(`${BACKEND_URL}/settings/legal`, {
          headers: { Authorization: `Bearer ${session.accessToken}` },
        }),
      ]);
  ...
  const handleUpdate = async (type: string, payload: any) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          `${type.charAt(0).toUpperCase() + type.slice(1)} configuration updated`,
        );
        fetchData();
      } else {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-8">
        <div className="space-y-1">
          ...
          <p className="text-slate-500 text-sm">
            Manage physical operational parameters and legal frameworks.
          </p>
        </div>
            ...
            <Card className="shadow-sm border-slate-200">
              <CardHeader className="border-b bg-slate-50/50">
                ...
                <CardDescription className="text-xs">
                  Define operational availability nodes for each solar cycle.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-0">
                <div className="divide-y divide-slate-100">
                  {DAYS.map((day) => (
                    <div
                      key={day.key}
                      className="flex flex-col md:flex-row md:items-center justify-between p-6 gap-6 hover:bg-slate-50/30 transition-colors"
                    >
                      <div className="flex items-center gap-6 min-w-[200px]">
                        ...
                        />
                        <Label className="text-sm font-bold uppercase tracking-tight text-slate-700">
                          {day.label}
                        </Label>
                      </div>
                      ...
                      <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-50 border border-slate-100">
                        <div
                          className={`h-1.5 w-1.5 rounded-full ${businessHours[day.key]?.isOpen ? 'bg-emerald-500' : 'bg-slate-300'}`}
                        />
                ...
                <div className="p-6 bg-slate-50/50 border-t">
                  <Button
                    onClick={() =>
                      handleUpdate('contact', { ...contact, businessHours })
                    }
                    disabled={isSaving}
              ...
              <Card key={policy.key} className="shadow-sm border-slate-200">
                ...
                <CardContent className="p-6 space-y-4">
                  <textarea
                    ...
                    onChange={(e) =>
                      setLegal({ ...legal, [policy.key]: e.target.value })
                    }
