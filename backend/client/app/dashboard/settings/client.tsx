...
export default function SettingsClient() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [genRes, curRes, conRes] = await Promise.all([
        ...
        fetch(`${BACKEND_URL}/settings/currency`, {
          headers: { Authorization: `Bearer ${session.accessToken}` },
        }),
  ...
  const handleUpdate = async (type: string, payload: any) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          `${type.charAt(0).toUpperCase() + type.slice(1)} settings updated`,
        );
        fetchData();
      } else {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="border-b pb-8">
        ...
        <p className="text-slate-500 text-sm mt-1">
          Manage your store identity, localization, and contact preferences.
        </p>
      </div>
      ...
      >
        <TabsList className="bg-slate-100/50 p-1 border rounded-lg h-auto flex flex-wrap lg:inline-flex w-full lg:w-auto">
          <TabsTrigger
            value="identity"
            className="rounded-md px-6 py-2 text-xs font-bold uppercase tracking-wider data-[state=active]:bg-white data-[state=active]:shadow-sm"
          >
                ...
                <Card className="shadow-sm border-slate-200">
                  <CardHeader className="border-b bg-slate-50/50">
                    ...
                    <CardDescription className="text-xs">
                      Define your public brand identity and SEO metadata.
                    </CardDescription>
                  </CardHeader>
              ...
              <div className="space-y-6">
                <Card className="shadow-sm border-slate-200">
                  <CardHeader className="border-b bg-slate-50/50 py-4">
                    <CardTitle className="text-sm font-bold uppercase tracking-wider text-slate-600 italic">
                      Visual Assets
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="p-6 space-y-6">
                ...
                <Card className="shadow-sm border-slate-200">
                  <CardContent className="p-4 flex items-center justify-between">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase font-black italic">
                        Public Access Guard
                      </p>
                    </div>
                    <Switch
                      ...
                      onCheckedChange={(val) =>
                        setGeneral({ ...general, maintenanceMode: val })
                      }
                    />
                  </CardContent>
                </Card>
          ...
          >
            <Card className="shadow-sm border-slate-200">
              <CardHeader className="border-b bg-slate-50/50">
                ...
                <CardDescription className="text-xs">
                  Primary communication nodes for customer synergy.
                </CardDescription>
              </CardHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold flex items-center gap-2 italic">
                      <Mail className="w-3.5 h-3.5 text-blue-500" /> Public
                      Email
                    </Label>
                    <Input
                      value={contact.email || ''}
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold flex items-center gap-2 italic">
                      <MapPin className="w-3.5 h-3.5 text-rose-500" /> Physical
                      Address
                    </Label>
                    <Input
            ...
            <div className="mb-6 flex items-center justify-between p-5 rounded-xl border border-primary/20 bg-primary/5 shadow-sm">
              <div className="space-y-1">
                <h3 className="text-sm font-black text-primary uppercase flex items-center gap-2">
                  <Languages className="w-5 h-5" /> Multi-Language & Translation
                  Matrix
                </h3>
                <p className="text-xs text-primary/70 font-medium tracking-wide">
                  Manage storefront languages, RTL support, and use AI to
                  auto-generate UI translations.
                </p>
              </div>
              <Link href="/dashboard/settings/languages">
                <Button
                  size="sm"
                  className="h-10 px-6 font-bold uppercase tracking-wider text-[11px] hover:scale-105 transition-transform shadow-md shadow-primary/20"
                >
            ...
            <Card className="shadow-sm border-slate-200">
              <CardHeader className="border-b bg-slate-50/50">
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    <CardTitle className="text-lg font-bold flex items-center gap-2">
                      <Globe className="w-5 h-5 text-blue-600" /> Localization
                      Matrix
                    </CardTitle>
                    <CardDescription className="text-xs">
                      Synchronize temporal and fiscal parameters.
                    </CardDescription>
                  </div>
                  <Button
                    variant="outline"
                  ...
                  <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8 p-6 bg-slate-50/50 rounded-xl border border-slate-100">
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          setGeneral({ ...general, dateFormat: v })
                        }
                      >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          setGeneral({ ...general, timeFormat: v })
                        }
                      >
                        <SelectTrigger className="h-11 bg-white border-slate-200">
                          <SelectValue />
          ...
          >
            <Card className="shadow-sm border-slate-200">
              <CardHeader className="border-b bg-slate-50/50">
                ...
                <CardDescription className="text-xs">
                  Manage your brand's presence across major platforms.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {[
                    ...
                    {
                      key: 'instagram',
                      icon: <Instagram className="w-4 h-4 text-rose-500" />,
                      label: 'Instagram',
                    },
                    {
                      key: 'twitter',
                    ...
                    {
                      key: 'tiktok',
                      icon: <Share2 className="w-4 h-4 text-slate-900" />,
                      label: 'TikTok',
                    },
                    {
                      key: 'linkedin',
