...
export default function NewBlogPostPage() {
  ...
  const fetchLanguages = async () => {
    try {
      ...
      if (res.success && res.data && res.data.length > 0) {
        ...
        // Initialize translations mapping for any custom languages
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetchApi<{ success: boolean; message: string }>('/blog', {
        method: 'POST',
        headers: {
      ...
      <div className="flex items-center justify-between border-b pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">Write and publish premium blog posts.</p>
          </div>
        </div>
