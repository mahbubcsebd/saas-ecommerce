...
export default function RolesClient() {
    ...
    const handleDelete = async (id: string) => {
        if (!await confirm({
            title: 'Delete Role',
            message: 'Are you sure you want to delete this role archetype? This will remove permissions for all assigned personnel.',
            type: 'danger',
    ...
    return (
        <div className="space-y-12 animate-in fade-in duration-700 pb-24">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-12">
                    <div className="space-y-6">
                        <div className="inline-flex items-center gap-2 px-6 py-2 rounded-full bg-slate-900 text-white text-[10px] font-black uppercase tracking-[0.2em] shadow-xl">
                            <ShieldCheck className="w-4 h-4 text-blue-400" /> Granular Access Control
                        </div>
                        ...
                        <p className="text-slate-500 font-bold max-w-2xl text-xl leading-relaxed">
                            Architect custom operational roles by bridging specific organizational modules with precise capability strings.
                        </p>
                    </div>
                    ...
                    >
                        <Plus className="w-6 h-6 mr-3 group-hover:rotate-90 transition-transform duration-500" /> Architect New Role
                    </Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {roles.map(role => (
                    <Card key={role.id} className="group rounded-[2.5rem] border-none shadow-[0_16px_48px_-8px_rgba(0,0,0,0.04)] hover:shadow-[0_32px_64px_-16px_rgba(37,99,235,0.12)] bg-white transition-all duration-500 hover:-translate-y-2 overflow-hidden flex flex-col">
                        ...
                        <CardHeader className="p-10 pb-4">
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0">
                                    <Button onClick={() => openEditModal(role)} variant="ghost" size="icon" className="h-10 w-10 rounded-xl hover:bg-blue-50 text-blue-600">
                                        <Edit2 className="w-4 h-4" />
                                    </Button>
                                    <Button onClick={() => handleDelete(role.id)} variant="ghost" size="icon" className="h-10 w-10 rounded-xl hover:bg-rose-50 text-rose-500">
                            ...
                            <CardDescription className="text-sm font-bold text-slate-400 italic">
                                {role.description || 'No specialized narrative provided for this role.'}
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="p-10 pt-0 mt-auto">
                            <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-300 mb-1">Assigned Personnel</span>
                                    <span className="text-lg font-black text-slate-900">{role._count?.users || 0} Members</span>
                                </div>
                                <div className="flex flex-col text-right">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-300 mb-1">Capacities</span>
                                    <span className="text-lg font-black text-blue-600">{role.permissions.length} nodes</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                ...
                <DialogContent className="max-w-5xl rounded-[3rem] p-0 border-none shadow-[0_64px_128px_-32px_rgba(0,0,0,0.25)] overflow-hidden bg-slate-50">
                    <div className="flex h-[85vh]">
                        ...
                        <div className="w-[350px] bg-white p-12 flex flex-col gap-10 border-r border-slate-100 shadow-xl">
                            ...
                            <div className="pt-8 border-t border-slate-100 space-y-4">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Status Quo</span>
                                    <Badge className="bg-emerald-50 text-emerald-600 border-none font-black rounded-lg">Operational</Badge>
                                </div>
                                <Button
                                    onClick={handleSave}
                        ...
                        <div className="flex-1 overflow-y-auto p-12 scrollbar-hide">
                            <div className="mb-12">
                                <h3 className="text-4xl font-[1000] italic uppercase tracking-tighter text-slate-900 mb-2">Capability Matrix</h3>
                                <p className="text-slate-400 font-bold italic">Assign specialized modular capacities to this role archetype.</p>
                            </div>
                                ...
                                {MODULES.map(module => (
                                    <div key={module.id} className="space-y-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <h4 className="text-xs font-[1000] uppercase tracking-[0.2em] text-slate-900">{module.label}</h4>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className={`h-10 w-10 rounded-xl flex items-center justify-center transition-all ${
                                                            isSelected ? 'bg-blue-600 text-white' : 'bg-slate-50 text-slate-300 group-hover:bg-blue-50 group-hover:text-blue-600'
                                                        }`}>
                                                            {isSelected ? <CheckCircle2 className="w-5 h-5" /> : <action.icon className="w-5 h-5" />}
                                                        </div>
                                                        <div className="space-y-1">
                                                            <p className={`text-[10px] font-black uppercase tracking-widest transition-colors ${isSelected ? 'text-slate-900' : 'text-slate-400'}`}>
                                                                {action.label}
                                                            </p>
                                                            <p className="text-[9px] font-bold text-slate-300 uppercase italic">{perm}</p>
                                                        </div>
                                                    </div>
                                                );
                                            })}
