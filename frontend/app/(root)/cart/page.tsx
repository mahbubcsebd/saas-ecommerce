    ...
    return (
      <div className="container py-20 flex flex-col items-center justify-center gap-4 text-center">
        <h2 className="text-2xl font-bold">
          {t('common', 'cartEmpty', { defaultValue: 'Your Cart is Empty' })}
        </h2>
        <p className="text-muted-foreground">
          {t('common', 'cartEmptyDesc', {
            defaultValue: "Looks like you haven't added anything yet.",
          })}
        </p>
        <Button asChild>
          <Link href="/">
            {t('common', 'continueShopping', { defaultValue: 'Continue Shopping' })}
          </Link>
        </Button>
      </div>
    );
    ...
    <div className="container py-10">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold">
          {t('common', 'shoppingCart', { defaultValue: 'Shopping Cart' })}
        </h1>
        ...
        {cart.items.length > 0 && (
          <div className="flex items-center gap-4">
            ...
            <Button variant="destructive" size="sm" onClick={clearCart} className="h-8 group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-4">
          {cart.items.map((item) => {
            ...
            const displayStock =
              (item.variant ? (item.variant as any).stock : item.product.stock) || 0;
            ...
            return (
              ...
              >
                ...
                <Link
                  ...
                  onClick={(e) => e.stopPropagation()}
                  className="relative h-20 w-20 shrink-0 overflow-hidden rounded-md border bg-muted group-hover:opacity-90 transition-opacity"
                >
                  {displayImage && (
                    <Image
                      src={displayImage}
                ...
                <div className="flex flex-1 flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex flex-col gap-1 min-w-[200px] flex-1">
                    <Link
                      href={`/products/${item.product.slug}`}
                      onClick={(e) => e.stopPropagation()}
                      className="hover:text-primary transition-colors text-sm font-medium line-clamp-2 w-fit"
                    >
                      {item.product.name}
                    </Link>
                    {item.variant && (
                  ...
                  >
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (val > displayStock) {
                          toast.error(
                            t('product', 'onlyLeftShort', {
                              count: String(displayStock),
                              defaultValue: `Only ${displayStock} available in stock.`,
                            })
                          );
                        }
        ...
        <div className="lg:col-span-1">
          <div className="rounded-lg border bg-card p-6 shadow-sm sticky top-20">
            ...
            <div className="space-y-2 mb-6 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">
                  {t('common', 'subtotal', { defaultValue: 'Subtotal' })} ({selectedItemIds.length}{' '}
                  {t('common', 'itemsCount', 'items')})
                </span>
                <span>{formatPrice(subtotal)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">
                  {t('common', 'shipping', { defaultValue: 'Shipping' })}
                </span>
                <span>
                  {t('common', 'shippingCalculated', { defaultValue: 'Calculated at checkout' })}
                </span>
              </div>
            ...
            <Button
              ...
              asChild={selectedItemIds.length > 0}
            >
              {selectedItemIds.length > 0 ? (
                ...
                <span>{t('common', 'selectItemsToCheckout', 'Select items to checkout')}</span>
              )}
            </Button>
          </div>
        </div>
