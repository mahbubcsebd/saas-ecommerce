...
export default function InvoicePage() {
  ...
  if (error)
    return <div className="p-10 text-red-500 text-center border m-10 rounded-lg">{error}</div>;
  ...
  return (
    <div className="container py-10 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 no-print bg-secondary/20 p-4 rounded-lg border">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <FileText className="h-6 w-6" /> Invoice #{order.orderNumber}
          </h1>
      ...
      {/* A4 Invoice Preview */}
      <div
        className="invoice-print-area bg-white text-black p-0 md:p-12 border shadow-lg mx-auto max-w-[210mm] min-h-[297mm] relative"
        id="invoice-section"
      >
        {/* Header Section */}
        <div className="flex justify-between items-start border-b-2 border-primary/20 pb-8 mb-8">
          <div className="space-y-4">
            ...
            <div className="text-sm text-gray-500 space-y-1">
              ...
              <p className="flex items-center gap-2">
                <Globe className="h-3 w-3" /> {companyInfo.website}
              </p>
            </div>
          </div>
          <div className="text-right">
            <h2 className="text-4xl font-extrabold text-primary/10 tracking-widest leading-none">
              INVOICE
            </h2>
            <div className="mt-4 space-y-1">
              ...
              <p className="text-sm text-gray-500">
                Status:{' '}
                <span className="uppercase font-medium text-black bg-gray-100 px-2 py-0.5 rounded text-xs">
                  {order.paymentStatus}
                </span>
              </p>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="text-right py-4 pr-4 align-top font-medium text-gray-900">
                      {formatPrice(
                        (item.salePrice || item.price || item.unitPrice) * item.quantity
                      )}
                    </td>
                  </tr>
                );
          ...
          <div className="grid grid-cols-2 gap-8 text-left mb-8">
            <div>
              ...
              <p className="text-xs leading-relaxed">
                Payment is due within 15 days. Please check the goods upon delivery. Returns
                accepted within 7 days with original receipt.
              </p>
            </div>
