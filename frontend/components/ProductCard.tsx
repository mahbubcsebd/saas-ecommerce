    ...
    >
      ...
      >
        ...
        <div className="absolute top-2 left-2 z-10 flex flex-col gap-1">
            ...
            {product.stock <= 0 && (
                <Badge variant="destructive" className="px-2 py-0.5">{t('common', 'outOfStock', { defaultValue: 'Out of Stock' })}</Badge>
            )}
        </div>
        ...
        <div>
            <div className="mb-2 flex items-center justify-between">
                ...
                <div className="flex flex-col items-end">
                        <span className="text-base font-bold text-red-600">{formatPrice(product.sellingPrice)}</span>
            ...
            <Link href={`/${product.slug}`} className="block group/title">
                <h3 className={cn("text-base font-semibold transition-colors group-hover/title:text-primary leading-tight mb-1", layout === 'grid' && "line-clamp-1")}>
                {localizedName}
                </h3>
            </Link>
            <p className={cn("text-xs text-muted-foreground leading-relaxed", layout === 'grid' ? "line-clamp-2" : "line-clamp-3 mb-4")}>
