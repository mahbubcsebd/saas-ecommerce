    ...
    <footer className="border-t bg-background">
      <div className="container py-12 md:py-16">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-4">
          <div className="space-y-4">
            {settings?.general?.footerLogo ? (
              <Link href="/" className="inline-block mb-4">
                <img
                  src={settings.general.footerLogo}
                  alt={settings.general.siteName}
                  className="h-10 w-auto"
                />
              </Link>
            ) : (
              <h3 className="text-lg font-bold">{settings?.general?.siteName || 'Mahbub Shop'}</h3>
            )}
            <p className="text-sm text-muted-foreground max-w-xs">
              {settings?.general?.tagline
                ? t('common', 'footerTagline', { defaultValue: settings.general.tagline })
                : t('common', 'footerTagline', {
                    defaultValue: 'Your one-stop destination for premium products.',
                  })}
            </p>
            ...
            <ul className="space-y-2 text-sm">
              ...
              <li>
                <Link href="/products" className="text-muted-foreground hover:text-primary">
                  {t('common', 'featuredProducts', { defaultValue: 'Products' })}
                </Link>
              </li>
              <li>
                <Link href="/categories" className="text-muted-foreground hover:text-primary">
                  {t('common', 'shopByCategory', { defaultValue: 'Categories' })}
                </Link>
              </li>
              <li>
                <Link href="/about" className="text-muted-foreground hover:text-primary">
                  {t('common', 'aboutUs', { defaultValue: 'About Us' })}
                </Link>
              </li>
            </ul>
          </div>
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-medium">
              {t('common', 'customerService', { defaultValue: 'Customer Service' })}
            </h3>
            <ul className="space-y-2 text-sm">
              <li>
              ...
              <li>
                <Link href="/profile" className="text-muted-foreground hover:text-primary">
                  {t('common', 'myAccount', { defaultValue: 'My Account' })}
                </Link>
              </li>
              <li>
                <Link href="/profile/orders" className="text-muted-foreground hover:text-primary">
                  {t('common', 'trackOrder', { defaultValue: 'Track Order' })}
                </Link>
              </li>
              <li>
              ...
              <li>
                <Link href="/terms" className="text-muted-foreground hover:text-primary">
                  {t('common', 'termsConditions', { defaultValue: 'Terms and Conditions' })}
                </Link>
              </li>
            </ul>
          </div>
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              {settings?.contact?.addressLine1 && (
                <li>
                  {settings.contact.addressLine1}, {settings.contact.city}
                </li>
              )}
              {settings?.contact?.country && (
                <li>
                  {t('common', settings.contact.country, {
                    defaultValue: settings.contact.country,
                  })}
                </li>
              )}
              {settings?.contact?.email && (
                <li>
                  {t('common', 'email')}: {settings.contact.email}
                </li>
              )}
              {settings?.contact?.phone && (
                <li>
                  {t('common', 'phone')}: {settings.contact.phone}
                </li>
              )}
            </ul>
          </div>
        ...
        <div className="mt-12 border-t pt-8 text-center text-sm text-muted-foreground">
          <p>
            {settings?.general?.copyrightText ||
              t('common', 'footerCopyright').replace('2026', currentYear.toString())}
          </p>
        </div>
      </div>
    </footer>
