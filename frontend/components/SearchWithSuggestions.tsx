    ...
    useEffect(() => {
        const fetchResults = async () => {
            ...
            try {
                ...
                // Fetch Products
                const prodRes = await fetch(`${API_URL}/products?search=${encodeURIComponent(debouncedQuery)}&limit=5`);
                const prodData = await prodRes.json();
                if (prodData.success) setProducts(prodData.data);

                // Fetch Categories
                const catRes = await fetch(`${API_URL}/categories?search=${encodeURIComponent(debouncedQuery)}`);
                                                ...
                                                {categories.map((category) => (
                                                    <li key={category.id}>
                                                        ...
                                                        >
                                                            ...
                                                            <span className="text-sm font-medium">{getLocalized(category, locale, 'name')}</span>
                                                        </Link>
                                                    </li>
                                                ))}
                                            </ul>
