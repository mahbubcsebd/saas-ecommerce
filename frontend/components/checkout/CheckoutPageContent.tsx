...
export default function CheckoutPageContent({ session, initialAddress }: CheckoutPageContentProps) {
  ...
  if (cartLoading) return <div className="p-20 text-center">{t('common', 'loadingCart', { defaultValue: 'Loading cart...' })}</div>;
  ...
  if (checkoutItems.length === 0) {
      return <div className="p-20 text-center">No items selected for checkout. Please return to the cart.</div>;
  }
  ...
  const initialData = {
    ...
    lastName: initialAddress?.name?.split(" ").slice(1).join(" ") || session?.user?.name?.split(" ").slice(1).join(" ") || "",
        ...
        <div>
          <div className="mb-6">
            ...
            <p className="text-sm text-muted-foreground">
              {session ? t('common', 'reviewDetails', { defaultValue: 'Review your details' }) : t('common', 'enterShippingInfo', { defaultValue: 'Enter your shipping information' })}
            </p>
          </div>
